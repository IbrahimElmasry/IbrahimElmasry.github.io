(() => {
  ...
  const revealTargets = '.section-heading, .project-card, .timeline-item, .stack-intro, .skill-group, .contact-layout > div, .contact-section > .section-kicker, .service-card, .process-card, .about-grid > div, .pillar-card, .approach-banner, .faq-accordion-item, .proof-strip, .arch-explorer-card, .telemetry-card, .recipe-vault-card, .roi-calculator-card';
  let observer;
  let navObserver;
  const init = () => {
